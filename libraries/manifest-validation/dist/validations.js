...
export const maniTests = [
        ...
        test: (value) => {
            ...
            if (isArray) {
                const noWebp = value.every((shortcut) => {
                    if (!shortcut.icons)
                        return true;
                    return shortcut.icons.every((icon) => {
        ...
        test: (value) => {
            ...
            if (isArray) {
                const has96x96Icon = value.every((shortcut) => {
                    if (!shortcut.icons)
                        return true;
                    return shortcut.icons.some((icon) => {
    ...
    },
    {
        ...
        quickFix: true,
        test: (value) => value && typeof value === "string" && value.length > 0 && isValidLanguageCode(value)
    },
    {
        member: "dir",
        ...
        quickFix: true,
        test: (value) => value && typeof value === "string" && value.length > 0 && (value === "ltr" || value === "rtl" || value === "auto")
    },
    {
        member: "description",
    ...
    },
    {
        ...
        test: (value) => {
            const isArray = value && Array.isArray(value);
            if (isArray) {
                const allValid = value.every((protocolHandler) => {
...
export async function loopThroughKeys(manifest) {
...
export async function loopThroughRequiredKeys(manifest) {
    return new Promise((resolve) => {
        let data = [];
        const keys = Object.keys(manifest);
        keys.forEach((key) => {
...
export async function findSingleField(field, value) {
