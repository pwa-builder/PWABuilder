 ...
 * message, cmd, stdout, stderr, and stack properties.
 *
 ...
 * passed as arguments - in the thrown error's message, cmd, stdout, and stderr properties.
...
export function redactSecretsFromError<T>(error: T, secrets: (string | undefined | null)[]): T {
    const secretValues = secrets.filter((secret): secret is string => !!secret);
