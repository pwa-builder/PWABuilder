...
export class AzureStorageBlobService implements BlobStorage {
    ...
    private readonly containerName = "meta-horizon-packages";
    ...
    /**
     * Uploads a file to the meta-horizon-packages container in PWABuilder's Azure Storage account.
     */
    ...
    /**
     * Downloads a blob from the meta-horizon-packages container as a stream
     */
...
class LocalFileSystemBlobService implements BlobStorage {
    ...
    async uploadFile(filePath: string, blobName: string): Promise<string> {
        ...
        return new Promise((resolve) => setTimeout(() => resolve(blobName), 1000));
    }
