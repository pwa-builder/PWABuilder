...
unsignedBtn.addEventListener("click", () => setCode(getUnsignedPackageOptions()));
signedBtn.addEventListener("click", () => setCode(getSignedPackage()));
existingKeyBtn.addEventListener("click", () => setCode(getExistingKeySignedPackage()))
chooseKeyBtn.addEventListener("click", () => filePicker.click());
filePicker.addEventListener("change", (e) => keyFileChosen(e));
submitBtn.addEventListener("click", () => submit());

setCode(getSignedPackage());
...
function getUnsignedPackageOptions() {
    return {
        ...
        "fullScopeUrl": "https://webboard.app/",
        "minSdkVersion": 23,
        "shortcuts": [
            {
...
function getSignedPackage() {
    const options = getUnsignedPackageOptions();
...
function getExistingKeySignedPackage() {
    const options = getSignedPackage();
...
async function submit() {
    ...
    try {
        ...
        if (!response.ok) {
            appendResults(`Failed to enqueue Meta Horizon packaging job. Status code ${response.status}, Error: ${response.statusText}.`);
            setLoading(false);
            return;
        }
