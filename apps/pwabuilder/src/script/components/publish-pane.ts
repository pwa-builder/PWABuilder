...
export class PublishPane extends LitElement {
  ...
  @state() storeMap: any = {
    Windows: {
      ...
      packaging_text:
        'Click below for instructions on how to submit to the Windows Store.',
    ...
    iOS: {
      ...
      packaging_text:
        'Click below for instructions on how to submit to the Apple App Store.',
      package_instructions: 'https://docs.pwabuilder.com/#/builder/app-store',
    },
    Meta: {
      logo: '/assets/meta_icon.svg',
      packaging_text:
        'Click below for instructions on how to submit to the Meta Quest Store.',
  ...
  renderForm() {
    if (this.selectedStore === 'Windows') {
      return html`<windows-form
        id="packaging-form"
        .generating=${this.generating}
        .userProject=${this.userProject}
      ></windows-form>`;
    } else if (this.selectedStore === 'Android') {
      return html` <div id="apk-tabs">
          <div class="tab-holder selected-apk">
            <p class="apk-type" @click=${(e: any) => this.toggleApkType(e)}>
              Google Play
            </p>
          </div>
          <div class="tab-holder unselected-apk">
            <p
              class="apk-type"
              id="other-android"
              @click=${(e: any) => this.toggleApkType(e)}
            >
              Other Android
            </p>
            <div id="unsigned-tooltip">
              <img src="/assets/new/tooltip.svg" alt="info circle tooltip" />
              <span class="toolTip"> Generates an unsigned APK. </span>
            </div>
          </div>
        </div>
        ${this.isGooglePlay
  ...
  async generate(platform: Platform, options?: PackageOptions) {
    ...
    try {
      ...
      //If the user is logged in, we store package info
      if (isUserLoggedIn()) {
        console.log('This is the selected store', this.selectedStore);
        storePackageOptionsForProject(options, platform, getURL());
        //update the user object with package options
      }
  ...
  renderErrorMessage(err: any) {
    ...
    if (err.message === 'Failed to fetch') {
      ...
      stack_trace += errString.slice(errString.indexOf(' at ') + 1);
      ...
      <div class="feedback-holder type-error">
        ...
        <div class="error-info">
          ...
          <div class="error-actions">
            <button
              class="copy_stack"
              @click=${() => this.copyText(stack_trace)}
            >
              Copy stack trace
            </button>
            <a
              href="https://docs.pwabuilder.com/#/builder/faq"
              target="_blank"
              rel="noopener"
              >Visit FAQ</a
            >
        ...
        <img
          @click=${() => (this.feedbackMessages = [])}
          class="close_feedback"
          src="assets/images/Close_desk.png"
          alt="close icon"
        />
      </div>
    `;
  ...
  renderContentCards(): TemplateResult[] {
    return this.platforms.map(
      (platform) => html` <div class="card-wrapper">
        ${true
          ? html``
          : html` <div class="packaged-tracker">
              <!-- This will eventually be in an "if packaged previously" -->
  ...
  backToCards() {
    ...
    recordPWABuilderProcessStep(
      `left_${this.selectedStore}_form`,
      AnalyticsBehavior.ProcessCheckpoint
    );
  }
      ...
      return html`
        <div id="form-extras">
          <div id="form-details-block">
            <p>
              ${this.isGooglePlay
                ? 'Click below for instructions on how to submit to the Google Play Store.'
                : 'Click below for instructions on how to submit to other Android stores.'}
            </p>
              ...
              <a
                @click=${() =>
                  recordPWABuilderProcessStep(
                    `${
                      this.isGooglePlay
                        ? this.selectedStore.toLowerCase()
                        : `other_${this.selectedStore.toLowerCase()}`
                    }_packaging_instructions_clicked`,
                    AnalyticsBehavior.ProcessCheckpoint
                  )}
                href=${this.isGooglePlay
                  ? 'https://docs.pwabuilder.com/#/builder/android'
                  : 'https://docs.pwabuilder.com/#/builder/other-android'}
                target="_blank"
                rel="noopener"
                >Packaging Instructions</a
              >
              <img
          ...
          <div id="form-options-actions" class="modal-actions">
            <sl-button
              ...
              ?disabled=${this.feedbackMessages.length > 0}
            >
              Download Package
            </sl-button>
            <a
        ...
        <div id="form-details-block">
          ...
          <div class="arrow_link">
            <a
              @click=${() =>
                recordPWABuilderProcessStep(
                  `${this.selectedStore.toLowerCase()}_packaging_instructions_clicked`,
                  AnalyticsBehavior.ProcessCheckpoint
                )}
              href=${this.storeMap[this.selectedStore].package_instructions}
              target="_blank"
              rel="noopener"
              >Packaging Instructions</a
            >
  ...
  submitForm() {
    let platForm = this.shadowRoot!.getElementById(
      'packaging-form'
    ) as AppPackageFormBase; // windows-form | android-form | ios-form | oculus-form
    ...
    if (form!.checkValidity()) {
      ...
      this.generate(
        this.selectedStore.toLowerCase() as Platform,
        packagingOptions
      );
    } else {
      form!.reportValidity();
            ...
            ${this.cardsOrForm
              ? html`
                  <div id="pp-frame-header">
                    ...
                    <p>
                      ...
                      Generate store-ready packages for the Microsoft Store,
                      Google Play, iOS and Meta stores.
                    </p>
                  </div>
              ...
              : html`
                  <div id="pp-form-header">
                    <button type="button">
                      <img
                        src="/assets/new/back_for_package_form.svg"
                        alt="back to store cards button"
                        @click=${() => this.backToCards()}
                      />
                    </button>
                    <div id="pp-form-header-content">
                      <img
                        src="${this.storeMap[this.selectedStore].logo}"
                        alt="${this.selectedStore} logo"
                      />
                  ...
                  <div id="form-area" data-store=${this.selectedStore}>
                    ${this.renderForm()}
                    <div id="feedback">
                      ${this.feedbackMessages.length > 0
                        ? this.feedbackMessages.map(
