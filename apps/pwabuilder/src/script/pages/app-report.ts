...
import {
  ...
  createManifestContextFromEmpty,
...
export class AppReport extends LitElement {
  ...
  @property({ type: Object }) CardStyles = {
    backgroundColor: 'white',
    color: 'black',
  };
  @property({ type: Object }) BorderStyles = {
  ...
  private possible_messages = [
    {
      messages: {
        green:
          'PWABuilder has analyzed your Web Manifest and your manifest is ready for packaging! Great job you have a perfect score!',
        yellow:
          'PWABuilder has analyzed your Web Manifest and your manifest is ready for packaging! We have identified recommeneded and optional fields that you can include to make your PWA better. Use our Manifest Editor to edit and update those fields.',
        blocked:
          'PWABuilder has analyzed your Web Manifest. You have a one or more fields that need to be updated before you can pacakge. Use our Manifest Editor to edit and update those fields. You can package for the store once you have a valid manifest.',
    ...
    },
    {
      messages: {
        ...
        yellow:
          'PWABuilder has analyzed your Service Worker, and has identified additonal features you can add, like offline support, to make your app feel more robust.',
        blocked: '',
    ...
    },
    {
      messages: {
        green:
          'PWABuilder has done a basic analysis of your HTTPS setup and found no issues! Great job you have a perfect score!',
        yellow: '',
        blocked: '',
  ...
  async populateAppCard(manifestContext: ManifestContext, url: string) {
    ...
    if (manifestContext && !this.createdManifest) {
      ...
      if (
        manifestContext.manifest.theme_color &&
        manifestContext.manifest.theme_color !== 'none'
      ) {
        this.CardStyles.backgroundColor = manifestContext.manifest.theme_color;
        // calculate whether is best to use white or black
        let color = this.pickTextColorBasedOnBgColorAdvanced(
          manifestContext.manifest.theme_color,
          '#ffffff',
          '#000000'
        );
        ...
        color === '#ffffff'
          ? (this.retestPath = '/assets/new/retest-white.svg')
          : '/assets/new/retest-black.svg';
      }
    } else {
      this.appCard = {
  ...
  async runAllTests(url: string) {
    ...
    if (isUserLoggedIn()) {
      ...
      let result = {
        manifestTotalScore: this.manifestValidCounter,
        swScore: this.swValidCounter,
        secTotalScore: this.secValidCounter,
      };
      await this.storeTestResults(result, url);
      this.runningTests = false;
    }
  }

  private async storeTestResults(result: any, url: string) {
    console.log('Storing user project', this.userProject);
    this.userProject.url = url;
    this.userProject.lastTested = new Date().toDateString();
    this.userProject.maniResult = JSON.stringify(result.manifestTotalScore);
    this.userProject.swResult = JSON.stringify(result.swScore);
    this.userProject.secResult = JSON.stringify(result.secTotalScore);

    storeUserProject(this.userProject);
  }
  ...
  async testManifest() {
    ...
    if (!this.createdManifest) {
      manifest = JSON.parse(
        sessionStorage.getItem('PWABuilderManifest')!
      ).manifest;
      ...
      this.todoItems.push({
        card: 'mani-details',
        field: 'Open Manifest Modal',
        fix: 'Edit and download your created manifest (Manifest not found before detection tests timed out)',
        status: 'red',
      });
    }
      ...
      if (result.result) {
        ...
        if (result.category === 'required') {
          ...
          this.todoItems.push({
            card: 'sw-details',
            field: 'Open SW Modal',
            fix: 'Add Service Worker to Base Package (SW not found before detection tests timed out)',
            status: status,
          });
  ...
  async openManifestEditorModal() {
    let dialog: any = this.shadowRoot!.querySelector(
      'manifest-editor-frame'
    )!.shadowRoot!.querySelector('.dialog');

    await dialog!.show();
    recordPWABuilderProcessStep(
      'manifest_editor_opened',
      AnalyticsBehavior.ProcessCheckpoint
    );
  }
  ...
  async openSWSelectorModal() {
    let dialog: any =
      this.shadowRoot!.querySelector('sw-selector')!.shadowRoot!.querySelector(
        '.dialog'
      );
  ...
  async openPublishModal() {
    let dialog: any =
      this.shadowRoot!.querySelector('publish-pane')!.shadowRoot!.querySelector(
        '.dialog'
      );
  ...
  iconSrcListParse(icon: any) {
    ...
    let iconURL: string =
      this.handleImageUrl(icon, manifest, manifestURL) || '';
  ...
  async animateItem(e: CustomEvent) {
    ...
    let itemList = this.shadowRoot!.querySelectorAll(
      '[data-field="' + e.detail.field + '"]'
    );
  ...
  addRetestTodo(toAdd: string) {
    this.todoItems.push({
      card: 'retest',
      field: 'Manifest',
      fix: 'Add ' + toAdd + ' to your server and retest your site!',
      status: 'retest',
      displayString: toAdd,
    });
    this.requestUpdate();
  ...
  rotateZero(card: string) {
    recordPWABuilderProcessStep(
      card + '_details_expanded',
      AnalyticsBehavior.ProcessCheckpoint
    );
    let icon: any = this.shadowRoot!.querySelector(
      '[data-card="' + card + '"]'
    );
    if (icon) {
  ...
  renderIndicators() {
    ...
    if (yellow + red != 0) {
      return html` <div id="indicators-holder">
        ${red != 0
  ...
  render() {
    return html`
        ...
        <div id="content-holder">
          <div id="header-row">
            ${this.isAppCardInfoLoading
              ? html` <div id="app-card" class="flex-col skeleton-effects">
                  <div id="app-card-header">
                    <sl-skeleton
                      id="app-image-skeleton"
                      effect="pulse"
                    ></sl-skeleton>
                    ...
                    <sl-skeleton
                      class="app-info-skeleton skeleton-desc"
                      effect="pulse"
                    ></sl-skeleton>
                  </div>
                  <div id="app-card-footer">
                    <sl-skeleton
                      class="app-info-skeleton-half"
                      effect="pulse"
                    ></sl-skeleton>
                  </div>
                </div>`
              : html` <div
                  id="app-card"
                  class="flex-col"
                  style=${this.createdManifest
                    ? styleMap({ backgroundColor: 'white', color: '#595959' })
                    : styleMap(this.CardStyles)}
                >
                  <div id="app-card-header">
                    <div id="pwa-image-holder">
                      ${this.proxyLoadingImage
                        ? html`<span class="proxy-loader"></span>`
                        : html`<img
                            src=${this.appCard.iconURL}
                            alt=${this.appCard.iconAlt}
                          />`}
                    </div>
                    <div id="card-info" class="flex-col">
                  ...
                  <div
                    id="app-card-footer"
                    style=${styleMap(this.BorderStyles)}
                  >
                    <div id="test" style=${styleMap(this.CardStyles)}>
                      ...
                      >
                        <p
                          id="last-edited"
                          style=${styleMap(this.LastEditedStyles)}
                        >
                          ${this.lastTested}
                        </p>
            ...
            <div id="app-actions" class="flex-col">
              <div id="package" class="flex-col-center">
                  ...
                  : html`
                      <sl-tooltip class="mani-tooltip">
                        ${this.runningTests
                          ? html`<div
                              slot="content"
                              class="mani-tooltip-content"
                            >
                              <img
                                src="/assets/new/waivingMani.svg"
                                alt="Waiving Mani"
                              />
                              <p>Running tests...</p>
                            </div>`
                          : html`<div
                            ...
                            >
                              ...
                              />
                              <p>
                                Your PWA is not store ready! Check your
                                To-do-list and handle all required items!
                              </p>
                            </div>`}
                        <button
                          type="button"
                ...
                <button
                  type="button"
                  id="test-download"
                  @click=${() => this.openTestPublishModal()}
                >
          ...
          <div id="todo">
            ...
            >
              ...
              this.todoItems.length > this.pageSize
                ? html` <div id="pagination-actions">
                    <button
                    ...
                    <div id="dots">
                      ${this.getDots().map((_dot: any, index: number) =>
                        this.pageNumber == index + 1
                          ? html`
                              <img
                                src="/assets/new/active_dot.svg"
                                alt="active dot"
                              />
                            `
                          : html`
                              <img
                                src="/assets/new/inactive_dot.svg"
                                alt="inactive dot"
                              />
                            `
                      )}
          ...
          <div id="manifest" class="flex-col">
            <div id="manifest-header">
                  ...
                  ${this.manifestDataLoading
                    ...
                    : html`
                        ${this.createdManifest
                          ? html` <sl-tooltip class="mani-tooltip" open>
                              <div slot="content" class="mani-tooltip-content">
                                ...
                                />
                                <p>
                                  We did not find a manifest on your site before
                                  our tests timed out so we have created a
                                  manifest for you! <br />
                        ...
                        <a
                          ...
                          @click=${() =>
                            recordPWABuilderProcessStep(
                              'manifest_documentation_clicked',
                              AnalyticsBehavior.ProcessCheckpoint
                            )}
                        >
              ...
              <div id="mh-right">
                ${this.manifestDataLoading
                  ? html`<sl-skeleton
                      class="progressRingSkeleton"
                      effect="pulse"
                    ></sl-skeleton>`
                  : html`<sl-progress-ring
                      id="manifestProgressRing"
                      class=${classMap(this.decideColor('manifest'))}
                      value="${this.createdManifest
                        ? 0
                        : (parseFloat(
                            JSON.stringify(this.manifestValidCounter)
                          ) /
                            this.manifestTotalScore) *
                          100}"
                      >${this.createdManifest
                        ? html`<img
                            src="assets/new/macro_error.svg"
                            class="macro_error"
                            alt="missing manifest requirements"
                          />`
                        : html`${this.manifestValidCounter} /
                          ${this.manifestTotalScore}`}</sl-progress-ring
                    >`}
              </div>
            </div>
            <sl-details
            ...
            >
              ${this.manifestDataLoading
                ? html`<div slot="summary">
                    <sl-skeleton
                      class="summary-skeleton"
                      effect="pulse"
                    ></sl-skeleton>
                  </div>`
                : html`<div class="details-summary" slot="summary">
                    <p>View Details</p>
                    <img
                      class="dropdown_icon"
                      data-card="mani-details"
                      src="/assets/new/dropdownIcon.svg"
                      alt="dropdown toggler"
                    />
                  </div>`}
              <div id="manifest-detail-grid">
                <div class="detail-list">
                  ...
                  ${this.requiredMissingFields.length > 0
                    ? html`
                        ${this.requiredMissingFields.map(
                          (field: string) =>
                            html`<div class="test-result" data-field=${field}>
                              <sl-tooltip
                                content=${field +
                                ' is missing from your manifest.'}
                                placement="right"
                              >
                                <img
                                  src=${stop_src}
                                  alt="invalid result icon"
                                />
                              </sl-tooltip>
                      ...
                      `
                    : html``}
                  ${this.validationResults.map((result: Validation) =>
                    result.category === 'required' ||
                    (result.testRequired && !result.valid)
                      ? html`
                          <div class="test-result" data-field=${result.member}>
                            ${result.valid
                              ? html`<img
                                  src=${valid_src}
                                  alt="passing result icon"
                                />`
                              : html`<sl-tooltip
            ...
            <div id="sw" class="half-width-cards">
                ...
                <div id="swh-top">
                  ...
                  ${this.swDataLoading
                    ...
                    : html`<sl-progress-ring
                        ...
                        value="${(parseFloat(
                          JSON.stringify(this.swValidCounter)
                        ) /
                          this.swTotalScore) *
                        100}"
                        >${this.swValidCounter == 0
                          ? html`<img
                              src="assets/new/macro_error.svg"
                              class="macro_error"
                              alt="missing service worker requirements"
                            />`
                          : html`${this.swValidCounter} / ${this.swTotalScore}`}</sl-progress-ring
                      > `}
                </div>
                  ...
                  ${this.swDataLoading
                    ...
                    : html`
                        <button
                          type="button"
                          class="alternate"
                          @click=${() => this.openSWSelectorModal()}
                        >
              ...
              >
                ...
                <div class="detail-grid">
                  <div class="detail-list">
                    <p class="detail-list-header">Required</p>
                    ${this.serviceWorkerResults.map((result: TestResult) =>
                      result.category === 'required'
                        ? html`
                            <div
                              class="test-result"
                              data-field=${result.infoString}
                            >
                              ${result.result
                                ? html`<img
                                    src=${valid_src}
            ...
            <div id="security" class="half-width-cards">
                ...
                <div id="sec-top">
                  ...
                  ${this.secDataLoading
                    ...
                    : html`<sl-progress-ring
                        ...
                        value="${(parseFloat(
                          JSON.stringify(this.secValidCounter)
                        ) /
                          this.secTotalScore) *
                        100}"
                        >${this.secValidCounter == 0
                          ? html`<img
                            ...
                            />`
                          : html`${this.secValidCounter} / ${this.secTotalScore}`}</sl-progress-ring
                      > `}
                </div>
                  ...
                  ${this.secDataLoading
                    ...
                    : html`
                        <a
                          ...
                          @click=${() =>
                            recordPWABuilderProcessStep(
                              'security_documentation_clicked',
                              AnalyticsBehavior.ProcessCheckpoint
                            )}
                        >
              ...
              >
                ...
                <div class="detail-grid">
                  <div class="detail-list">
                    <p class="detail-list-header">Required</p>
                    ${this.securityResults.map((result: TestResult) =>
                      result.category === 'required'
                        ? html`
                            <div
                              class="test-result"
      ...
      <sl-dialog
        class="dialog"
        ?open=${this.showConfirmationModal}
        @sl-hide=${() => (this.showConfirmationModal = false)}
        noHeader
      >
        ${this.retestConfirmed
          ? html` <p>Retesting your site now!</p> `
          : html`
      ...
      ${this.manifestDataLoading
        ? html``
        : html`<manifest-editor-frame
            .isGenerated=${this.createdManifest}
            @readyForRetest=${() => this.addRetestTodo('Manifest')}
          ></manifest-editor-frame>`}
      <sw-selector
        @readyForRetest=${() => this.addRetestTodo('Service Worker')}
      ></sw-selector>
    `;
  }
}
