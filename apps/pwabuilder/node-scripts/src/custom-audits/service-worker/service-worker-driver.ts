...
type WorkerRegistrationUpdatedEvent =
	LH.Crdp.ServiceWorker.WorkerRegistrationUpdatedEvent;

export async function getServiceWorkerVersions(
	session: ProtocolSession
): Promise<WorkerVersionUpdatedEvent> {
	return new Promise((resolve, reject) => {
		const versionUpdatedListener = (data: WorkerVersionUpdatedEvent) => {
			...
			const activateCandidates = data.versions.filter(
				(sw) => sw.status !== 'redundant'
			);
			const hasActiveServiceWorker = activateCandidates.find(
				(sw) => sw.status === 'activated'
			);
			if (!activateCandidates.length || hasActiveServiceWorker) {
				...
				session
					.sendCommand('ServiceWorker.disable')
...
export async function getServiceWorkerRegistrations(
	session: ProtocolSession
): Promise<WorkerRegistrationUpdatedEvent> {
	return new Promise((resolve, reject) => {
		session.once(
			'ServiceWorker.workerRegistrationUpdated',
			(data: WorkerRegistrationUpdatedEvent) => {
				session
					.sendCommand('ServiceWorker.disable')
					.then((_) => resolve(data), reject);
			}
