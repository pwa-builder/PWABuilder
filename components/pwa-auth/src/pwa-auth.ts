...
export class PwaAuthImpl extends LitElement implements PwaAuth {
  ...
  @property({ type: String }) credentialMode: 'none' | 'silent' | 'prompt' =
    'silent';
  @property({ type: Boolean }) menuOpened = false;
  @property({ type: Boolean }) logOutFlyoutOpened = false;
  @property({ type: String, reflect: true }) menuPlacement: 'start' | 'end' =
    'start';
  @property({ type: Boolean }) disabled = false;
  @property({ type: Boolean }) isLoggedIn = false;
  @property({ type: String }) loggedInProvider:
    | null
    | 'Microsoft'
    | 'Google'
    | 'Facebook'
  ...
  readonly providers: ProviderInfo[] = [
    {
      ...
      signOut: () => this.signOut('Microsoft'),
    },
    {
      name: 'Google',
  ...
  static styles = css`
    ...
    .signout-btn {
      ...
      background-color: var(
        ...
        var(--background-color-sub1, #faf9f8)
      );
      border: none;
      padding: 0px;
      cursor: pointer;
  ...
  async tryAutoSignInOnLoad() {
    ...
    if (credential && provider) {
      await this.trySignInWithStoredCredential(provider.url);
    }
    await this.trySignInWithStoredCredential('https://graph.microsoft.com');
    if (credential) {
  ...
  public signIn(providerName: ProviderName): Promise<SignInResult> {
    ...
    return this.signInWithProvider(provider).then((result) =>
      this.signInCompleted(result)
    );
  }

  public async signOut(providerName: ProviderName): Promise<void> {
    const provider = this.providers.find((p) => p.name === providerName);
    if (provider) {
      return this.signOutWithProvider(provider);
    }
  }
  ...
  private renderLoginButton(): TemplateResult {
    return html`
      <div class="dropdown" @focusout="${this.dropdownFocusOut}">
        ...
        <div
          class="menu ${this.logOutFlyoutOpened ? 'open' : ''} ${this
            .menuPlacement === 'end'
            ? 'align-end'
            : ''}"
          part="dropdownMenu"
        >
          ${this.renderLogOutFlyout()}
        </div>
      </div>
    `;
  }

  private renderLogOutFlyout(): TemplateResult {
    return html`<div>
      <button
        class="signout-btn"
        @click=${() => {
        ...
        .map(
          (provider) => html`
            <div class="provider" part="${provider.containerPartName}">
              <button
                class="${provider.btnClass}"
                ?disabled=${this.disabled}
                part="${provider.buttonPartName}"
                @click="${provider.signIn}"
              >
                <img
                  part="${provider.iconPartName}"
                  loading="${this.iconLoading}"
                  width="20px"
                  height="20px"
                  src="${provider.getIconUrl()}"
                />
  ...
  private renderNoKeysError(): TemplateResult {
    return html`<div class="provider-error">
      <strong>❌ No available sign-ins</strong><br /><em
        >To enable sign-in, pass a Microsoft key, Google key, Facebook, or Apple
        key to the &lt;pwa-auth&gt; component.</em
      ><br />
      <pre>&lt;pwa-auth microsoftkey="..."&gt;&lt;/pwa-auth&gt;</pre>
    </div>`;
  }
  ...
  private async signOutWithProvider(provider: ProviderInfo) {
    console.log('Inside sign out with provider', provider.getKey());
    this.logOutFlyoutOpened = false;
    this.disabled = true;
    const key = provider.getKey();
    if (provider.signOut && key) {
      const importedProvider = await provider.import(key);
      //@ts-ignore
  ...
  private async signInWithProvider(
    ...
    if (this.disabled) {
      return Promise.reject(
        'Sign-in already in progress, rejecting new sign-in attempt'
      );
    }
  ...
  private importMicrosoftProvider(key: string): Promise<SignInProvider> {
    return import('./microsoft-provider').then(
      (module) => new module.MicrosoftProvider(key)
    );
  }
  ...
  private importFacebookProvider(key: string): Promise<SignInProvider> {
    return import('./facebook-provider').then(
      (module) => new module.FacebookProvider(key)
    );
  }

  private importAppleProvider(key: string): Promise<SignInProvider> {
    return import('./apple-provider').then(
      (module) => new module.AppleProvider(key, this.appleRedirectUri)
    );
  }
    ...
    if (signIn.email && federatedCredentialCtor) {
      try {
        const cred = new federatedCredentialCtor({
          id: signIn.email,
          provider:
            this.providers.find((p) => p.name === signIn.provider)?.url ||
            signIn.provider,
          name: signIn.name || '',
  ...
  private async tryAutoSignIn(): Promise<FederatedCredential | null> {
    ...
    if (this.credentialMode === 'prompt') {
      ...
      credential = await this.getStoredCredential(
        'required',
        this.providers.map((p) => p.url)
      );
  ...
  private trySignInWithStoredCredential(
    ...
    return this.getStoredCredential('silent', [providerUrl])
      .catch((error) =>
        console.warn(
          'Error attempting to sign-in with stored credential',
          error
        )
      )
      .then((credential) =>
        credential ? this.credentialToSignInResult(credential) : null
      );
  }
  ...
  private credentialToSignInResult(cred: FederatedCredential): SignInResult {
    console.log('Signing in using credential management');
    return {
      name: cred.name,
      email: cred.id,
      providerData: null,
  ...
  private loadAllDependencies(): Promise<any> {
    ...
    return Promise.all(dependencyLoadTasks).catch((error) =>
  ...
  private tryReadStoredTokenInfo(
    ...
    try {
      ...
      console.warn(
        'Unable to read auth token from local storage',
        localStorageKey,
        error
      );
      return null;
