...
export default Ember.Object.extend({
    isAlias: function(color) {
        return typeof colorConstants.create().colorsAlias[color.toLowerCase()] !== 'undefined';
    },
    ...
    fromAlias: function (color) {
        ...
        var hexColor = colorConstants.create().colorsAlias[color.toLowerCase()];
    ...
    fromRGB: function(color) {
        ...
        var channels = this.rgbRegExp.exec(color);
        if (channels === null) {
            throw new Error('The color passed has not a valid format: ' + channels);
        }

        var hexaColor = "#" + this.getHexaWithPadding(channels[1]) + this.getHexaWithPadding(channels[2]) + this.getHexaWithPadding(channels[3]);
    ...
    fromHSL: function(color) {
        ...
        var channels = this.hslRegExp.exec(color);
        if (channels === null) {
            throw new Error('The color passed has not a valid format: ' + channels);
        }
        ...
        var rgb = this.hslToRgb(channels[1], channels[3], channels[5]);

        var hexaColor = "#" + this.getHexaWithPadding(rgb.r) + this.getHexaWithPadding(rgb.g) + this.getHexaWithPadding(rgb.b);
        return this.fromHexadecimal(hexaColor, alphaChannel);    
    },
