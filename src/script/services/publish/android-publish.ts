...
export async function createAndroidPackageOptionsFromForm(form: HTMLFormElement, signingFile?: string): Promise<AndroidApkOptions> {
  ...
  const icon =
    ...
    findSuitableIcon(manifestIcons, 'maskable', 512, 512, 'image/jpeg') ||
    findSuitableIcon(manifestIcons, 'any', 192, 192, 'image/jpeg') ||
    findSuitableIcon(manifestIcons, 'maskable', 192, 192, 'image/jpeg') ||
    findSuitableIcon(manifestIcons, 'any', 512, 512, undefined) || // A 512x512 or larger maskable image with unspecified type
