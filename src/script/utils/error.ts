...
export function getReportErrorUrl(errorMessage: string | Object, platform: string): string {
  if (!errorMessage) {
  ...
  const errorMessageTrimmed = errorMessage.toString().substring(0, gitHubErrorMessageMaxLength);
  const title = encodeURIComponent(`Error generating ${platform} package`);
  const message = encodeURIComponent(
    `I received the following error when generating a package for ${getURL() || "my app"
    }\n\n${errorMessageTrimmed}`
  );
