...
test('known good pwa Windows package generation test', async ({ page }) => {
  ...
  const response = await page.waitForResponse(response => response.url() === 'https://pwabuilder-win-chromium-platform.centralus.cloudapp.azure.com/msix/generatezip' && response.status() === 200);
