...
test('choosing a SW base_package generation test', async ({ page }) => {
  ...
  const sw_button = await page.evaluateHandle(`document.querySelector("body > fast-design-system-provider > app-index").shadowRoot.querySelector("#router-outlet > app-report").shadowRoot.querySelector("#sw")`);
  await sw_button.click();

  // checking that we opened our SW picker tab
  expect(await page.isVisible('text=Secret Ingredient: A Service Worker')).toBeTruthy();
  ...
  const sw_done_button = await page.evaluateHandle(`document.querySelector("body > fast-design-system-provider > app-index").shadowRoot.querySelector("#router-outlet > app-report").shadowRoot.querySelector("#swPanel > sw-picker").shadowRoot.querySelector("#header-actions > app-button").shadowRoot.querySelector("fast-button").shadowRoot.querySelector("button")`);
  ...
  const response = await page.waitForResponse((response) => {
    return response.url() === 'https://pwabuilder-web-platform.azurewebsites.net/form?siteUrl=https://www.example.com&swId=1&hasServiceWorker=false' && response.status() === 200
  });
